...
\subtitle{ Lecture 11. Approximation algorithm: a brief introduction
%\footnote{The slides are made based on Chapter 11 of {\it Algorithm design}, {\it Approximation Algorithm} by V. Vazirani, and {\it Combinatorial optimization  algorithm and complexity} by C. H. Papadimitriou and K. Steiglitz, and slides by D. P. Williamson. } 
}
\author{Dongbo Bu } 
...
We have a couple of options: 
 \begin{enumerate}
 \item Give up \textcolor{red}{\bf polynomial-time} in \textcolor{red}{\bf worst-case}  restriction:  though our algorithm takes exponential time in the worst case, we hope that the algorithm run fast on the practical instances,  e.g. branch-and-bound, branch-and-cut, and branch-and-pricing algorithms are used to solve a TSP instance with  over  24978 Swedish Cities \begin{small} (see http://www.tsp.gatech.edu/history/pictorial/sw24978.html) \end{small}
 \item Give up \textcolor{red}{\bf optimum} restriction: from \textcolor{red}{\bf optimal} solution to \textcolor{red}{\bf nearly optimal} solution in the hope that \textcolor{red}{\bf nearly optimal} is easy to find,  \begin{small} e.g. approximation algorithm (with theoretical guarantee), heuristics, local search (without theoretical guarantee; \end{small} 
 \item Give up \textcolor{red}{\bf deterministic} restriction: we hope that the expectation of running time of a randomized algorithm might be polynomial; 
 \end{enumerate}
} 
...
\begin{itemize}
\item \textbf{Key observation:} solution is a partition of jobs, or $X=(x_1, x_2, ..., x_n)$, where $x_i = 1, 2, ..., m$.  
 \item Basic idea: Let's describe the solving process as a series of decisions. At each decision step, we assign a job to a machine. 
...
\begin{itemize}
 ...
 \item Then for each $S_j$ that contains $e$ as a member, we have $x_j^* < \frac{1}{d}$; (Otherwise, $S_j$ should be selected by Algorithm1.)
...
\begin{Theorem}
 (Hochbaum '82) Algorithm1 is a $d$-approximation algorithm for {\sc SetCover}. 
\end{Theorem}
\begin{footnotesize}
