...
\item Thus, it transforms the complex vector $a_0, a_1,..., a_{n-1}$  into another complex vector $y_0, y_1, ..., y_{n-1}$, where $y_{i} = A( w^{i})$, i.e., 
\begin{small}
\[
...
\frame{ 
\frametitle{FFT: a fast way to implement DFT [Cooley-Tukey 1965]} 
...
\end{center}
\item FFT: reduce $O(n^2)$ to $O(n\log_2 n)$ using divide-and-conqueror technique. 
%\item How does FFT achieve this? Or what calculations are redundant in the direct matrix-vector multiplication approach?
\item Note: The idea of  FFT was proposed by Cooley and Tukey in 1965 when analyzing earth-quake data, but the idea can be dated back to F. Gauss. 
\end{itemize}
}
...
%
%\begin{itemize}
%	\item It is easy to evaluate any $1$-degree polynomial $A(x) = a_0 + a_1 x$ at two special point $1, -1$. \item Now let's consider evaluating  a $3$-degree  polynomial $A(x) = a_0 + a_1 x + a_2 x^2 + a_{3} x^{3}$ at four special points $1, -i, -1, i$. 
%	\item \textcolor{blue}{\bf Divide:} Break the polynomial into even and odd terms, i.e., 
%		\begin{itemize}
...
%			\item $A(-x) = A_{even}(x^2) - x A_{odd}(x^2)$
%		\end{itemize} 
%	\item \textcolor{blue}{\bf Combine:} For 4 special points $1, -i, i, -1$, we have 
%		\begin{itemize}
%			\item $A(1) = A_{even}(1) +  A_{odd}(1)$ 
...
%
%\def\d{0.1};
%\draw[thick, red] (-\d, \d) rectangle (-4.7, 1.6);
%\draw[thick, red] (-\d, -\d) rectangle (-4.7, -1.6);
%
%\def\e{0.68};
%\draw[thick, blue] (\e, \d) rectangle (5., 1.6);
%\draw[thick, blue] (\e, -\d) rectangle (5., -1.6);
%
%\node at (0,0) {
%$
...
%
%
%Note: the calculations in the two red frames are the same; and the two calculations in the blue frames are also identical after multiplying by $\omega^2$. 
% 
%}
%
%
...
%
%\def\d{0.1};
%\draw[green!15, fill=green!15] (-\d, \d) rectangle (-4.7, 1.6);
%%\draw[thick, red] (-\d, -\d) rectangle (-6.7, -2.2);
%
%\def\e{0.68};
%%\draw[thick, blue] (\e, \d) rectangle (7.8, 2.2);
%\draw[green!15, fill=green!15] (\e, -\d) rectangle (5. , -1.6);
%
%\node at (0,0) { 
%$
%\begin{array}{lllllllllllllllllllllllllllll}
...
%\end{figure}
%
%Note: the calculations in the top-left and  bottom-right frames are redundant. We need only $2+4+2=4\times \log_{2} 4$ calculations.
%
%}  
%
%
...
%
%
%Note: the calculations in the two red frames are the same; and the two calculations in the blue frames are also identical after multiplying by $\omega^4$. 
% 
%}
%
%
...
%\end{figure}
%
%Note: the calculations in the top-left and  bottom-right frames are redundant. 
%}  
%
%
%\frame{ 
...
%	\RETURN{ $a_0$ }; 
%\ENDIF
%\STATE{$(E_0, E_1,..., E_{\frac{n}{2} - 1} ) =$ {\sc FFT}$( \frac{n}{2}, a_0, a_2, ..., a_{n} );$ }
...
%$y_{i} = A(\omega^{i})$,  and $A(x) = a_{0} + a_{1} x + a_{2}x^{2}+...+a_{n-1}x^{n-1}$.  
%
%\item Matrix form
%\[
%\begin{bmatrix} y_0 \\  y_1 \\ \vdots \\ y_{n-1} \end{bmatrix}  = 
...
%	\RETURN{$y_0$ }; 
%\ENDIF
%\STATE{$(E_0, E_1,..., E_{\frac{n}{2} - 1} ) =$ {\sc IFFT}$( \frac{n}{2}, y_0, y_2, ..., y_{n} );$ }
...
%\end{algorithmic} 
%
%Note: here we assume $n$ is the power of $2$ for simplicity. The normalization factors multiplying FFT and IFFT (here 1 and $\frac{1}{n}$) and the signs of exponents are merely conventions, and differ in some treatments. 
%} 
...
\frame{
	\frametitle{$y_1$ encodes amplitude and phase of a sinusoidal component}

	\begin{itemize}
		\begin{small}
		\item $y_{1} = a_{0} +\   a_{1}\omega^{1}  + \   a_{2}\omega^{2}  +  \hdots  +\  a_{7}\omega^{7}$ computes the direct product of two vectors: the input data $a_0, a_1, ..., a_7$, and a sinusoid signal $1, \omega^{1}, \omega^{2}, ..., \omega^{7}$. Here $\omega=e^{\tfrac{2\pi}{8}i}$ and thus the sinusoid has frequency of $1$ cycle per $8$ samples.
		\item The direct product of vectors is essentially a discrete analogy of the integral of two sinusoids  $$  \int_{0}^{2\pi}  \cos mx \cdot \sin nx  \mathrm{d} x $$    

	\item The orthogonality of sinusoids states that 
	$$\int_{0}^{2\pi} \cos mx \cdot \cos nx \mathrm{d} x = \begin{cases} \tfrac{\pi}{n} & m=n \\ 
	0 & m \neq n \end{cases}$$ 
	\item Thus  $y_{1}  = 0$ if the input data $a_0, a_1, ..., a_7$ does not consist of a sinusoidal component of frequency $1$ cycle per $8$ samples. 
%			\begin{footnotesize}	
%				\begin{tt}
...
\frame{
	...
	\begin{itemize}
		\begin{small}
		\item As $1, \omega^{1}, \omega^{2}, ..., \omega^{7}$ represents a sinusoidal signal of frequency $1$ cycle per $8$ samples, the direct product $y_{1} = a_{0} +\   a_{1}\omega^{1}  + \   a_{2}\omega^{2}  +  \hdots  +\  a_{7}\omega^{7}$ could detect the existence of such sinusoidal component in the input data  $a_0, a_1, ..., a_7$. %Here $\omega=e^{\tfrac{2\pi}{8}  i}$.
...
\frame{
	...
	\begin{itemize}
		\item $1, \omega^{2}, \omega^{4}, ..., \omega^{14}$ represents a sinusoidal signal of frequency $2$ cycles per $8$ samples, and the existence of such sinusoidal component  in the input data  $a_0, a_1, ..., a_7$ is encoded by the direct product $y_{2} = a_{0} +\   a_{1}\omega^{2}  + \   a_{2}\omega^{4}  +  \hdots  +\  a_{7}\omega^{14}$. %Here $\omega=e^{\tfrac{2\pi}{8}  i}$.
		\item \begin{footnotesize}	
		\begin{tt}
a = [1 2.1 -2 0.7 -1 -2.1 2 -0.7] \\
c = [ 1 0 -1 0 1 0 -1 0 ] \\
...
\frame{
	...
	\begin{itemize}
		\item $1, \omega^{3}, \omega^{6}, ..., \omega^{21}$ represents a sinusoidal signal of frequency $3$ cycles per $8$ samples, and the existence of such sinusoidal component  in the input data  $a_0, a_1, ..., a_7$ is encoded by the direct product $y_{3} = a_{0} +\   a_{1}\omega^{3}  + \   a_{2}\omega^{6}  +  \hdots  +\  a_{7}\omega^{21}$. %Here $\omega=e^{\tfrac{2\pi}{8}  i}$.	
		\item \begin{footnotesize}	
		\begin{tt}
a = [1 2.1 -2 0.7 -1 -2.1 2 -0.7]  \\
c = [1 -0.7 0 0.7 -1 0.7 0 -0.7] \\
...
%
%\frame{
%	\frametitle{Orthogonality of basis functions}
%	
%	\begin{itemize}
%		\item Unlike Taylor's expansion, the basis functions of Fourier series are orthogonal over $[0, 2\pi]$, i.e.,
%\begin{small}
...
%	$$\int_{0}^{2\pi} \cos mx \cdot \cos nx \mathrm{d} x = \begin{cases} \tfrac{\pi}{n} & m=n \\ 
%	0 & m \neq n \end{cases}$$ 
% the dot product $y_m = a_0 + a_1 \omega^{m} + a_2 \omega^{2m} + ... + a_{n-1} \omega^{m(n-1)}$ is just a discrete analogy of  $\int_{0}^{2\pi} \cos mx \cdot \cos nx \mathrm{d} x$. 
...
%	
%	\begin{itemize}
%		\item For an approximate function $f(x)$ with its Fourier transform (in terms of ordinary frequency) \begin{footnotesize}$F(\nu) = {\displaystyle \int_{-\infty}^{\infty}} f(x) e^{-2\pi i x \nu} \mathrm{d} x$\end{footnotesize}, the Poisson summation formula states 
...
%			\begin{footnotesize}$ \sum\limits_{k=-\infty}^{\infty} f(k) = 1 = \sum\limits_{k=-\infty}^{\infty} F(k) $.\end{footnotesize}				
%	\end{itemize}
%		
% \begin{figure}
%
